...
//             String title=snapshot.data[index]['title']??'title';
//             String subTitle=snapshot.data[index]['body']??'body';
//             return ListTile(
//               title: Text(title),
        ...
        builder: (BuildContext context, AsyncSnapshot snapshot) {
          if (snapshot.hasError) {
            return Center(
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [Icon(Icons.error, color: Colors.red)],
              ),
            );
          }
          if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          if (snapshot.hasData) {
            return ListView.builder(
              itemBuilder: (context, index) {
                String title = snapshot.data[index]['title']?? 'title';
                String subTitle = snapshot.data[index]['body']?? 'body';
                return ListTile(title: Text(title.toString()), subtitle: Text(subTitle.toString()));
              },
            );
          }
          return Container();
